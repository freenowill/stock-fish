...
返回JSON格式的子问题列表。"""
    ...
    def interview_agents(
        ...
        # Step 4: 检查模拟环境是否存活，再调用真实的采访API
        if not SimulationRunner.check_env_alive(simulation_id):
            logger.warning(f"模拟环境 {simulation_id} 已关闭，跳过采访")
            result.summary = "模拟环境已关闭，采访不可用，使用知识图谱数据替代分析"
            try:
                ctx = self.search_graph(graph_id=self.graph_id, query=interview_requirement, limit=20)
                if ctx and ctx.to_text():
                    result.raw_interview_data = {"graph_fallback": True, "data": ctx.to_text()}
            except Exception:
                pass
            return result
